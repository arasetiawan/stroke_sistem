...
with col1:
    gender = 0 if st.radio('Apa Jenis kelamin Anda?', ['Perempuan', 'Laki-laki']) == 'Perempuan' else 1
    age = st.number_input('Berapa umur anda?', min_value=0, max_value=120)
    hypertension = 0 if st.radio('Apakah Anda Punya riwayat Hipertensi?', ['Tidak', 'Ya']) == 'Tidak' else 1
    heart_disease = 0 if st.radio('Apakah Anda Punya riwayat Jantung?', ['Tidak', 'Ya']) == 'Tidak' else 1
    ever_married = 0 if st.radio('Apakah anda pernah menikah?', ['Tidak', 'Ya']) == 'Tidak' else 1

with col2:
    work_type = st.radio('Pilih tipe pekerjaan Anda:', ['Pemerintahan', 'Anak-anak', 'Swasta', 'Wiraswasta'])
    Residence_type =  0 if st.radio('Pilih tipe tempat tinggal Anda', ['Pedesaan', 'Perkotaan']) == 'Pedesaan' else 1
    ...
    smoking_status = st.radio('Pilih status merokok Anda:', ['Tidak diketahui', 'Dahulu merokok', 'Tidak pernah merokok', 'Merokok'])
...
st.write(f"- **Usia:** {age}")
st.write(f"- **Penyakit Hipertensi:** {hypertension}")
